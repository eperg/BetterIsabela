...
Pulls the latest posts from the official PLGU Isabela Facebook Page, categorizes
...
1. **Get an Editor role on the page.** Ask the PLGU Isabela page admin (via Meta
   Business Suite → Settings → People) to add the BetterIsabela account/app as an
...
| Name              | Type         | Value                                               |
| ----------------- | ------------ | --------------------------------------------------- |
| `FTP_SERVER`      | Secret       | cPanel FTP host (e.g. `ftp.betterisabela.org`)      |
